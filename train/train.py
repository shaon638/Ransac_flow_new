...
def run(args) :
    ...
    #     raise RuntimeError("Not support gpu version currently...")
    ...
    for epoch in range(args.nEpochs):
        ...
        if df is not None and valPrecEpoch > bestPrec :
            ...
            outNet = os.path.join(args.outDir, 'checkPoint_Epoch{:d}'.format(epoch))
            pth = {}
            for key in list(network.keys()) : 
                pth[key] = network[key].state_dict()
...
if __name__ == '__main__':
    ...
    parser.add_argument('--trainImgDir', type=str, default = '/ssd_scratch/cvit/shaon/forms_train_org', help='RGB train image directory')
